...
<ConfirmDialog
	...
	requireInput={true}
	inputLabel="Type yes to permanently delete all items"
	onconfirm={() => {
...
>
	This will permanently delete all {navigation.trashEntries.length} item(s) in the trash. This action cannot be undone.
</ConfirmDialog>
		...
		>
			{#each navigation.trashEntries as entry (entry.id)}
				...
				>
					...
					<span
						class="block w-full truncate text-center text-[0.6rem] text-muted-foreground/60"
